# AUTO-GENERATED by generate_bindings.py — DO NOT EDIT MANUALLY.
# Re-run generate_bindings.py to update from the latest bindings graph.
from __future__ import annotations
from dataclasses import dataclass, field
from typing import Optional



@dataclass
class ChatChannelPermissionState:
    entity_id: int = 0
    chat_channel_entity_id: int = 0
    player_entity_id: int = 0
    identity: str = 0
    rank: int = 0

    @classmethod
    def from_row(cls, raw) -> 'ChatChannelPermissionState':
        if raw is None:
            return cls()
        return cls(
            entity_id=(raw.get('entity_id') if isinstance(raw, dict) else raw[0]),
            chat_channel_entity_id=(raw.get('chat_channel_entity_id') if isinstance(raw, dict) else raw[1]),
            player_entity_id=(raw.get('player_entity_id') if isinstance(raw, dict) else raw[2]),
            identity=((raw.get('identity') if isinstance(raw, dict) else raw[3])['__identity__'] if isinstance((raw.get('identity') if isinstance(raw, dict) else raw[3]), dict) else ((raw.get('identity') if isinstance(raw, dict) else raw[3])[0] if isinstance((raw.get('identity') if isinstance(raw, dict) else raw[3]), list) else (raw.get('identity') if isinstance(raw, dict) else raw[3]))),
            rank=(raw.get('rank') if isinstance(raw, dict) else raw[4])
        )
